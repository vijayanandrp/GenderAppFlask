...
class OsUtils(object):
    ...
    def check_directory_and_create_if_not_exist(self, path=None):
        if not path:
            self.log.info('Path cannot be None')
            return False
        if not os.path.isdir(path):
            os.mkdir(path=path)
            self.log.info('{} path not available. Creating it'.format(path))
            return True
        else:
            self.log.info('{} path already available'.format(path))
            return True
